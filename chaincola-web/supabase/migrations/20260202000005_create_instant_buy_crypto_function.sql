-- Create instant_buy_crypto function
-- CRITICAL: BUY = NGN → CRYPTO (DEBIT NGN only, CREDIT crypto only)
-- This function MUST ALWAYS debit (subtract) NGN from user balance
-- NEVER credit NGN when buying crypto
...
BEGIN
  ...
  -- Example: ₦10,000 - ₦2,000 = ₦8,000 (correct - DEBIT)
  ...
  BEGIN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'fix_version', 'v5_20260202_critical_debit_fix'
      )
    )
...
-- Add comment
COMMENT ON FUNCTION public.instant_buy_crypto IS 'Instant buy crypto function. CRITICAL FIX v5: CORE RULE - BUY = NGN → CRYPTO. ALWAYS DEBITS user EXACTLY (amount + fee) in NGN. NEVER credits NGN when buying crypto. Uses EXACT formula: crypto_amount = ngn_amount ÷ buy_price. Uses SELECT FOR UPDATE to lock rows. Uses user_wallets.ngn_balance as PRIMARY source of truth. All operations in single atomic transaction. Includes mandatory safety guards: new_balance MUST be less than current_balance (debit validation). If NGN balance increases during Buy, treats as critical failure and aborts.';
