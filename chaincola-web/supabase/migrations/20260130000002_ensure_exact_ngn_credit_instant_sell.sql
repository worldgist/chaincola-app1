-- CRITICAL FIX: Ensure instant sell functions ALWAYS credit user NGN wallet exactly the amount sold
-- Issue: Need to guarantee that users receive exactly (amount * rate * (1 - fee)) in NGN
-- Fix: Use SELECT FOR UPDATE to lock rows, read current balance atomically, and credit exactly v_ngn_amount
...
BEGIN
  ...
  BEGIN
    -- 1. Update user_wallets table - credit EXACTLY v_ngn_amount
    ...
    -- 4. Update system wallet - debit EXACTLY v_ngn_amount
    ...
    -- 5. Insert transaction record with EXACT amounts
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'instant_sell', true,
        'fix_version', 'v3_20260130_exact_credit'
      )
    )
...
-- Add comment
COMMENT ON FUNCTION public.instant_sell_crypto_v2 IS 'Atomically swaps crypto to NGN instantly. CRITICAL FIX v3: ALWAYS credits user EXACTLY (amount * rate * (1 - fee)) in NGN. Uses SELECT FOR UPDATE to lock rows and prevent race conditions. Uses user_wallets.ngn_balance as PRIMARY source of truth. All wallet tables updated consistently with exact amounts.';
