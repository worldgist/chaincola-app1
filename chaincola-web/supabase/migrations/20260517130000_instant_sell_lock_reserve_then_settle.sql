-- Instant sell: verify balance, reserve on wallet_balances (locked), then settle
-- (user debit, NGN credit, system_wallets pending/inventory + ngn_float) in one transaction.
-- Row locks (user_wallets, wallet_balances, system_wallets) serialize concurrent sells/sends.
  ...
  BEGIN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'fix_version', 'v8_20260517_lock_reserve_settle'
      )
    );
...
COMMENT ON FUNCTION public.instant_sell_crypto_v2(UUID, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL, DECIMAL) IS
  'Instant sell: lock user_wallets + wallet_balances + system_wallets; reserve on wallet_balances.locked; credit NGN; settle user crypto and treasury (pending-first internalize).';
