-- Fix instant_sell_crypto_v2 to update ALL wallet tables (user_wallets, wallet_balances, and wallets)
-- This ensures the app can read balances from any table
...
BEGIN
  ...
  -- Get current NGN balance from ALL tables and use maximum
  v_current_ngn_from_wallet_balances := COALESCE((
    SELECT balance FROM public.wallet_balances 
    WHERE user_id = p_user_id AND currency = 'NGN'
  ), 0);
  
  v_current_ngn_from_wallets := COALESCE((
    SELECT ngn_balance FROM public.wallets 
    WHERE user_id = p_user_id
  ), 0);
  
  -- Use maximum NGN balance from all sources
  v_new_user_ngn_balance := GREATEST(
    v_user_wallet.ngn_balance,
    v_current_ngn_from_wallet_balances,
    v_current_ngn_from_wallets
  ) + v_ngn_amount;
...
-- Add comment
COMMENT ON FUNCTION public.instant_sell_crypto_v2 IS 'Atomically swaps crypto to NGN instantly. Updates user_wallets, wallet_balances, and wallets tables, plus system_wallets and transactions. All in one transaction.';
