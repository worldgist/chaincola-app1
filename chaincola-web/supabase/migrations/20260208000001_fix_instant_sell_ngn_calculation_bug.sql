-- CRITICAL FIX: Fix NGN calculation bug in instant_sell_crypto_v2
-- Issue: User sold 0.02941980 SOL and should have received ₦3,656.23 but got ₦23,000.00 (6.3x more)
-- Root Cause: Function may be using wrong variable or multiplying incorrectly
-- Fix: Ensure EXACT formula: NGN = crypto_amount × sell_price × (1 - fee_percentage)
--      Add validation to prevent over-crediting
...
BEGIN
  ...
  -- CRITICAL: Calculate NGN using EXACT formula: NGN_AMOUNT = CRYPTO_AMOUNT_SOLD × SELL_PRICE × (1 - FEE)
  -- MUST use v_crypto_amount_sold (the EXACT amount sold) and p_rate (sell price)
  -- Formula: total_ngn = crypto_amount × rate
  --          fee = total_ngn × fee_percentage
  --          ngn_amount = total_ngn - fee
  ...
  -- CRITICAL VALIDATION: Prevent over-crediting bug
  -- Maximum expected NGN should be: crypto_amount × rate × 1.1 (allow 10% tolerance for fees/spreads)
  -- If calculated amount exceeds this, it's likely a bug
  v_expected_ngn_max := v_crypto_amount_sold * p_rate * 1.1;
  IF v_ngn_amount > v_expected_ngn_max THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      format('CRITICAL ERROR: Calculated NGN amount (%s) exceeds maximum expected (%s). This indicates a calculation bug. crypto_amount=%s, rate=%s',
  ...
  v_reference := format('SELL_%s_%s_%s', p_asset, 
    TO_CHAR(NOW(), 'YYYYMMDDHH24MISS'), SUBSTRING(v_transaction_id::TEXT, 1, 8));

  -- Perform atomic sell transaction
  BEGIN
    -- 1. Update user_wallets: DEBIT crypto, CREDIT NGN
    ...
    -- 2. Update wallet_balances for crypto (debit)
    ...
    -- 3. Update wallet_balances for NGN (credit)
    ...
    -- 4. Update system_wallets: CREDIT crypto inventory, DEBIT NGN float
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'calculation_formula', format('NGN = %s × %s × (1 - %s) = %s', 
          v_crypto_amount_sold, p_rate, p_fee_percentage, v_ngn_amount),
        'instant_sell', true,
  ...
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 
      ...
      0::DECIMAL, 
      NULL::JSONB,
      format('Sell failed: %s', SQLERRM)::TEXT;
  END;
END;
$$;

COMMENT ON FUNCTION public.instant_sell_crypto_v2 IS 'Instant sell crypto function. CRITICAL FIX v6: Fixed NGN calculation bug. Uses EXACT formula: NGN = crypto_amount × sell_price × (1 - fee_percentage). Added validation to prevent over-crediting. Uses SELECT FOR UPDATE to lock rows. Uses user_wallets.ngn_balance as PRIMARY source of truth. All operations in single atomic transaction.';
