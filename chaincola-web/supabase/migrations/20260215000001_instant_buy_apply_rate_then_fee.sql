-- Apply rate first, then platform fee. Debit user only the NGN amount they pay (no extra fee on top).
-- Formula: 1) At rate: crypto_gross = ngn_amount / rate. 2) Then fee: fee_ngn = ngn_amount * fee%, crypto_net = (ngn_amount - fee_ngn) / rate.
-- User is debited exactly p_ngn_amount (the amount they are spending).
...
BEGIN
  ...
  -- Apply rate first, then platform fee.
  -- Step 1: User pays p_ngn_amount (we debit exactly this).
  -- Step 2: Fee = p_ngn_amount * fee%. Crypto = (p_ngn_amount - fee) / rate = (p_ngn_amount / rate) * (1 - fee%).
  v_total_ngn_before_fee := p_ngn_amount;
  v_fee := ROUND(v_total_ngn_before_fee * p_fee_percentage, 2);
  v_ngn_to_debit := p_ngn_amount;  -- Debit only what user pays (no extra fee on top)
...
COMMENT ON FUNCTION public.instant_buy_crypto(UUID, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL) IS
  'Instant buy: apply rate first (crypto = ngn/rate), then platform fee (fee = ngn*fee%, crypto_net = (ngn - fee)/rate). Debits user exactly p_ngn_amount.';
