-- Instant buy: require available NGN (GREATEST user_wallets vs wallet_balances minus locked),
-- row-lock NGN wallet_balances, reserve debit on locked columns inside the settle block, then
-- debit NGN, credit crypto from system inventory, sync wallet_balances + wallets.
...
DECLARE
  ...
  v_uw_ngn DECIMAL(20, 2);
  v_wb_ngn DECIMAL(20, 2);
  v_wb_locked_before DECIMAL(20, 2);
  v_gross_ngn DECIMAL(20, 2);
  v_avail_ngn DECIMAL(20, 2);
...
BEGIN
  ...
  v_uw_ngn := COALESCE(v_user_wallet.ngn_balance, 0);
  ...
  VALUES (p_user_id, 'NGN', v_uw_ngn, 0, 0, NOW())
  ON CONFLICT (user_id, currency) DO NOTHING;

  SELECT
    COALESCE(wb.balance, 0),
    COALESCE(wb.locked, 0)
  INTO v_wb_ngn, v_wb_locked_before
  FROM public.wallet_balances wb
  WHERE wb.user_id = p_user_id AND wb.currency = 'NGN'
  FOR UPDATE;

  IF NOT FOUND THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB, 'Could not lock NGN wallet_balances row for instant buy'::TEXT;
    RETURN;
  END IF;

  v_gross_ngn := GREATEST(v_uw_ngn, v_wb_ngn);
  v_avail_ngn := v_gross_ngn - v_wb_locked_before;

  IF v_avail_ngn < v_ngn_to_debit THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      format(
        'Insufficient available NGN. Gross: ₦%s, locked in pending ops: ₦%s, available: ₦%s, required: ₦%s',
        v_gross_ngn, v_wb_locked_before, v_avail_ngn, v_ngn_to_debit
      )::TEXT;
    RETURN;
  END IF;
  ...
  v_new_user_ngn_balance := ROUND(v_gross_ngn - v_ngn_to_debit, 2);

  IF v_new_user_ngn_balance >= v_gross_ngn THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      format('CRITICAL ERROR: NGN balance would not decrease during Buy. Gross: ₦%s, New: ₦%s. Aborting.',
        v_gross_ngn, v_new_user_ngn_balance)::TEXT;
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    SET
      balance = v_gross_ngn,
      locked = COALESCE(wb.locked, 0) + v_ngn_to_debit,
      locked_balance = COALESCE(wb.locked_balance, 0) + v_ngn_to_debit,
      updated_at = NOW()
    WHERE wb.user_id = p_user_id
      AND wb.currency = 'NGN'
      AND (v_gross_ngn - COALESCE(wb.locked, 0)) >= v_ngn_to_debit;

    GET DIAGNOSTICS v_reserve_rows = ROW_COUNT;
    IF v_reserve_rows IS NULL OR v_reserve_rows <> 1 THEN
      RAISE EXCEPTION 'INSTANT_BUY_RESERVE_FAILED';
    END IF;
    ...
    IF v_verify_ngn_balance IS NULL OR v_verify_ngn_balance > v_gross_ngn THEN
    ...
    INSERT INTO public.wallet_balances (user_id, currency, balance, locked, locked_balance, updated_at)
    VALUES (p_user_id, p_asset, v_new_user_asset_balance, 0, 0, NOW())
    ...
    SET
      ...
      locked = GREATEST(0::numeric, COALESCE(locked, 0) - v_ngn_to_debit),
      locked_balance = GREATEST(0::numeric, COALESCE(locked_balance, 0) - v_ngn_to_debit),
      updated_at = NOW()
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'rate_then_fee', true,
        'ngn_gross_before', v_gross_ngn,
        'ngn_locked_before', v_wb_locked_before,
        'wallet_balances_ngn_reserved', v_ngn_to_debit,
        'fix_version', 'v10_20260519_instant_buy_available_ngn'
      )
    )
    ...
    IF v_new_user_ngn_balance > v_gross_ngn THEN
      RAISE EXCEPTION 'CRITICAL FAILURE: NGN balance increased during Buy. Gross: %, After: %, Debit: %',
        v_gross_ngn, v_new_user_ngn_balance, v_ngn_to_debit;
    END IF;
    ...
    RETURN QUERY SELECT
      ...
      jsonb_build_object(
        ...
        'ngn_gross_before', v_gross_ngn,
        'ngn_locked_before', v_wb_locked_before,
        'ngn_available_before', v_avail_ngn,
  ...
  EXCEPTION WHEN OTHERS THEN
    IF POSITION('INSTANT_BUY_RESERVE_FAILED' IN SQLERRM) > 0 THEN
      RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
        'Could not reserve NGN for buy (balance changed). Try again.'::TEXT;
    ELSE
...
COMMENT ON FUNCTION public.instant_buy_crypto(UUID, TEXT, DECIMAL, DECIMAL, DECIMAL, DECIMAL) IS
  'Instant buy: available NGN = GREATEST(user_wallets, wallet_balances NGN) minus NGN locked; reserve then debit NGN, credit crypto from system inventory, sync balances.';
