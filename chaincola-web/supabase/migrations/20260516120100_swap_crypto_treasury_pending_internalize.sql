-- swap_crypto: when user swaps FROM an asset, internalize system *_pending_inventory first
-- (same rule as instant_sell) so deposit credits + swap do not double-count hot *_inventory.
...
DECLARE
  ...
  v_from_pend_before DECIMAL(20, 8);
  v_from_take DECIMAL(20, 8);
  v_new_from_pend DECIMAL(20, 8);
...
BEGIN
  ...
  -- Get system wallet (single treasury row)
  ...
  CASE UPPER(p_from_asset)
    WHEN 'BTC' THEN v_from_pend_before := COALESCE(v_system_wallet.btc_pending_inventory, 0);
    WHEN 'ETH' THEN v_from_pend_before := COALESCE(v_system_wallet.eth_pending_inventory, 0);
    WHEN 'USDT' THEN v_from_pend_before := COALESCE(v_system_wallet.usdt_pending_inventory, 0);
    WHEN 'USDC' THEN v_from_pend_before := COALESCE(v_system_wallet.usdc_pending_inventory, 0);
    WHEN 'XRP' THEN v_from_pend_before := COALESCE(v_system_wallet.xrp_pending_inventory, 0);
    WHEN 'SOL' THEN v_from_pend_before := COALESCE(v_system_wallet.sol_pending_inventory, 0);
    ELSE v_from_pend_before := 0;
  END CASE;
  ...
  -- System from-asset: consume pending first, remainder increases hot inventory
  v_from_take := LEAST(p_from_amount, COALESCE(v_from_pend_before, 0));
  v_new_from_pend := COALESCE(v_from_pend_before, 0) - v_from_take;
  v_new_system_from_inventory := ROUND(v_current_system_from_inventory + (p_from_amount - v_from_take), 8);
  ...
  BEGIN
    ...
    -- 4. Update system_wallets: credit from_asset (pending + inventory split), debit to_asset
    UPDATE public.system_wallets
    SET
      btc_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'BTC' THEN v_new_from_pend ELSE btc_pending_inventory END,
      eth_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'ETH' THEN v_new_from_pend ELSE eth_pending_inventory END,
      usdt_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'USDT' THEN v_new_from_pend ELSE usdt_pending_inventory END,
      usdc_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'USDC' THEN v_new_from_pend ELSE usdc_pending_inventory END,
      xrp_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'XRP' THEN v_new_from_pend ELSE xrp_pending_inventory END,
      sol_pending_inventory = CASE WHEN UPPER(p_from_asset) = 'SOL' THEN v_new_from_pend ELSE sol_pending_inventory END,
      btc_inventory = CASE 
...
COMMENT ON FUNCTION public.swap_crypto IS 'Swap one cryptocurrency for another in a single atomic transaction. From-asset treasury: consumes *_pending_inventory first (on-chain deposit bucket), then increases *_inventory for the remainder; debits to-asset hot inventory.';
