-- Fix instant_buy_crypto function to ensure fiat_currency is explicitly set to 'NGN'
-- Issue: Transactions were being recorded with wrong currency (USD instead of NGN)
-- This migration ensures fiat_currency is always set to 'NGN' for buy transactions
...
BEGIN
  ...
  BEGIN
    ...
    -- 2. Update wallet_balances: debit NGN (CRITICAL - ensure this happens)
    ...
    DO UPDATE SET 
      ...
      updated_at = NOW()
    WHERE wallet_balances.user_id = p_user_id AND wallet_balances.currency = 'NGN';
    ...
    DO UPDATE SET 
      ...
      updated_at = NOW()
    WHERE wallet_balances.user_id = p_user_id AND wallet_balances.currency = p_asset;
...
-- Add comment
COMMENT ON FUNCTION public.instant_buy_crypto IS 'Instant buy crypto function. Swaps NGN to crypto instantly using system inventory. Updates user_wallets, wallet_balances, and system_wallets tables. FIXED: Explicitly sets fiat_currency to NGN and ensures NGN balance is properly debited.';
