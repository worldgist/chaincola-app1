-- FORCE FIX: Ensure instant buy function ALWAYS debits NGN, never credits
-- This migration drops and recreates the function to ensure it's applied correctly
-- CRITICAL: BUY = NGN → CRYPTO (debit NGN only, credit crypto only)
...
BEGIN
  ...
  v_ngn_to_debit := v_total_ngn_before_fee + v_fee;
  
  -- Round to 2 decimal places for NGN
  v_ngn_to_debit := ROUND(v_ngn_to_debit, 2);
  v_fee := ROUND(v_fee, 2);
  
  -- Calculate crypto amount: crypto_amount = ngn_amount ÷ buy_price
  ...
  RAISE NOTICE 'BUY: NGN calculation: ₦% - ₦% = ₦%', v_current_user_ngn_balance, v_ngn_to_debit, v_new_user_ngn_balance;

  -- MANDATORY SAFETY GUARDS: Enforce core rule - BUY NEVER credits NGN
  -- Guard 1: New NGN balance MUST be less than current balance (we're subtracting)
  IF v_new_user_ngn_balance >= v_current_user_ngn_balance THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      format('CRITICAL ERROR: NGN balance would increase during Buy. This violates core rule: BUY = NGN → CRYPTO (debit NGN only). Current: ₦%s, New: ₦%s, Amount to debit: ₦%s. Aborting transaction.',
        v_current_user_ngn_balance, v_new_user_ngn_balance, v_ngn_to_debit)::TEXT;
    RETURN;
  END IF;
  
  -- Guard 2: crypto_amount MUST be greater than 0
  IF v_crypto_amount IS NULL OR v_crypto_amount <= 0 THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      format('CRITICAL ERROR: Invalid crypto_amount calculated. Amount: %s. Aborting transaction.', v_crypto_amount)::TEXT;
    RETURN;
  END IF;
  
  -- Guard 3: buy_price (rate) MUST be greater than 0
  IF p_rate IS NULL OR p_rate <= 0 THEN
    RETURN QUERY SELECT false, 0::DECIMAL, NULL::JSONB,
      'CRITICAL ERROR: Invalid buy price. Aborting transaction.'::TEXT;
    RETURN;
  END IF;
  ...
  BEGIN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'fix_version', 'v4_20260130_force_debit'
      )
    )
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'BUY TRANSACTION FAILED: error=%, ngn_balance_before=%, ngn_amount_debited=%, crypto_amount=%, asset=%',
      SQLERRM, v_current_user_ngn_balance, v_ngn_to_debit, v_crypto_amount, p_asset;
    RETURN QUERY SELECT 
      false,
      0::DECIMAL,
      NULL::JSONB,
...
-- Add comment
COMMENT ON FUNCTION public.instant_buy_crypto IS 'Instant buy crypto function. CRITICAL FIX v4: CORE RULE - BUY = NGN → CRYPTO. ALWAYS DEBITS user EXACTLY (amount + fee) in NGN. NEVER credits NGN when buying crypto. Uses EXACT formula: crypto_amount = ngn_amount ÷ buy_price. Uses SELECT FOR UPDATE to lock rows. Uses user_wallets.ngn_balance as PRIMARY source of truth. All operations in single atomic transaction. Includes mandatory safety guards and verification. If NGN balance increases during Buy, treats as critical failure and aborts.';
