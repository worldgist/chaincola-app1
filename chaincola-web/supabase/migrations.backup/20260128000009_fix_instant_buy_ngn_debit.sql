-- Fix instant_buy_crypto function to properly debit NGN balance
-- Issue: WHERE clause in ON CONFLICT DO UPDATE was preventing NGN balance from being debited
-- This migration removes the invalid WHERE clause and ensures NGN is properly debited
...
BEGIN
  ...
  -- Get user NGN balance from user_wallets (PRIMARY SOURCE OF TRUTH)
  -- Only use wallet_balances as fallback if user_wallets has NULL or 0 balance
  ...
  -- Get user asset balance from user_wallets (PRIMARY SOURCE OF TRUTH)
  CASE p_asset
  ...
  -- Only check wallet_balances if user_wallets balance is NULL or 0
  IF v_current_user_asset_balance IS NULL OR v_current_user_asset_balance = 0 THEN
    SELECT COALESCE(balance, 0) INTO v_current_asset_from_wallet_balances
    FROM public.wallet_balances
    WHERE user_id = p_user_id AND currency = p_asset
    ORDER BY updated_at DESC NULLS LAST, created_at DESC NULLS LAST
    LIMIT 1;
    
    IF v_current_asset_from_wallet_balances IS NOT NULL AND v_current_asset_from_wallet_balances > 0 THEN
  ...
  BEGIN
    -- 1. Update user_wallets: debit NGN, credit crypto
    ...
    -- 2. Update wallet_balances: debit NGN (CRITICAL FIX - removed invalid WHERE clause)
    ...
    -- Verify NGN balance was updated
    SELECT balance INTO v_current_ngn_from_wallet_balances
    FROM public.wallet_balances
    WHERE user_id = p_user_id AND currency = 'NGN';
    
    IF v_current_ngn_from_wallet_balances IS NULL OR v_current_ngn_from_wallet_balances != v_new_user_ngn_balance THEN
      RAISE EXCEPTION 'Failed to update NGN balance in wallet_balances. Expected: %, Got: %', 
        v_new_user_ngn_balance, COALESCE(v_current_ngn_from_wallet_balances, 0);
    END IF;

    -- 3. Update wallet_balances: credit crypto
    INSERT INTO public.wallet_balances (user_id, currency, balance, created_at, updated_at)
    ...
    -- 3.5. Update wallets table (CRITICAL - app reads from here first!)
    ...
    ON CONFLICT (user_id) 
    DO UPDATE SET 
      ngn_balance = EXCLUDED.ngn_balance, 
      updated_at = NOW();

    -- 4. Update system_wallets: debit crypto inventory, credit NGN float
    ...
    -- 5. Create transaction record - EXPLICITLY set fiat_currency to 'NGN'
    ...
    VALUES (
      ...
      v_crypto_amount,
      v_ngn_to_debit,
      'NGN', -- EXPLICITLY set to NGN (not relying on default)
      v_fee,
      p_fee_percentage * 100, -- Convert to percentage (e.g., 0.01 -> 1.00)
      'NGN', -- Fee currency is also NGN
      'COMPLETED',
      v_reference,
      NOW(),
      jsonb_build_object(
...
-- Add comment
COMMENT ON FUNCTION public.instant_buy_crypto IS 'Instant buy crypto function. Swaps NGN to crypto instantly using system inventory. Updates user_wallets, wallet_balances, wallets, and system_wallets tables. FIXED: (1) Removed invalid WHERE clause from ON CONFLICT DO UPDATE to ensure NGN balance is properly debited. (2) Fixed balance reading logic to use user_wallets as primary source of truth. (3) Added duplicate transaction prevention.';
