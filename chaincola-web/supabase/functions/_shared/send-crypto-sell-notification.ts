// Shared helper function to send push notifications for cryptocurrency sell transactions

import { NOTIFICATION_ICON } from "./notification-config.ts";

const EXPO_PUSH_API_URL = 'https://exp.host/--/api/v2/push/send';

interface SendCryptoSellNotificationParams {
  supabase: any;
  userId: string;
...
/**
 * Sends a push notification for a cryptocurrency sell transaction
 */
export async function sendCryptoSellNotification({
  supabase,
  userId,
  cryptoCurrency,
  cryptoAmount,
  ...
  sellId,
  status = 'COMPLETED',
}: SendCryptoSellNotificationParams): Promise<void> {
  try {
    ...
    if (status === 'COMPLETED') {
      title = `✅ ${cryptoCurrency} Sold Successfully`;
      statusMessage = 'Your sell order has been completed';
    } else if (status === 'PENDING') {
      title = `⏳ ${cryptoCurrency} Sell Pending`;
      statusMessage = 'Your sell order is being processed';
    } else if (status === 'FAILED') {
      title = `❌ ${cryptoCurrency} Sell Failed`;
      statusMessage = 'Your sell order failed';
    } else {
      title = `💰 ${cryptoCurrency} Sell Initiated`;
      statusMessage = 'Sell order placed';
    }

    // Create notification body
    const body = `${formatAmount(cryptoAmount, cryptoCurrency)} sold for ${formatNGN(ngnAmount)}. ${statusMessage}`;
    ...
    console.log(`📤 Sending ${messages.length} push notification(s) for ${cryptoCurrency} sell to user ${userId}`);
    ...
    console.error('❌ Exception sending crypto sell notification:', error);
  }
}
