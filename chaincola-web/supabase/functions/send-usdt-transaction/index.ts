// Send USDT Transaction Edge Function
// Sends USDT (ERC-20 token) using Alchemy API
//
// SECURITY: This is the ONLY function that decrypts ETH private keys for USDT transfers.
// Decryption happens ONLY when sending USDT transactions.
// Flow:
...
serve(async (req) => {
  ...
  try {
    ...
    const { destination_address, amount_usdt, skip_platform_fee } = body;
    ...
    if (!amount_usdt || parseFloat(amount_usdt) <= 0) {
      return new Response(
        JSON.stringify({ success: false, error: "amount_usdt is required and must be greater than 0" }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    // Get user's Ethereum wallet (same wallet used for ETH, since USDT is ERC-20 on Ethereum)
    ...
    if (balanceError || !balance) {
      return new Response(
        JSON.stringify({ success: false, error: "Failed to fetch USDT balance" }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    const availableBalance = parseFloat(balance.balance || '0');
    const amountToSend = parseFloat(amount_usdt);
    ...
    if (amountToSend > availableBalance) {
      return new Response(
        JSON.stringify({
          success: false,
          error: `Insufficient USDT balance. Available: ${availableBalance.toFixed(6)} USDT, Required: ${amountToSend.toFixed(6)} USDT`,
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    try {
      console.log('🔓 Decrypting private key for USDT transfer...');
      ...
      const tx = {
        to: USDT_CONTRACT_ADDRESS, // Contract address, not destination
      ...
      // Sign transaction
      console.log('✍️  Signing USDT transfer transaction...');
    ...
    // ============================================================
    console.log('📡 Broadcasting USDT transfer transaction...');
    ...
    console.log(`✅ USDT transfer transaction broadcasted: ${txHash}`);
    ...
    if (debitError) {
      console.error('⚠️ Failed to debit USDT balance:', debitError);
      // Don't fail the whole operation - transaction was sent
    } else {
      console.log(`✅ Debited ${amountToSend.toFixed(6)} USDT from balance`);
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        message: confirmed
          ...
          : 'USDT transfer sent. Confirmation pending.',
      }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error: any) {
    console.error('Send USDT transaction error:', error);
    return new Response(
