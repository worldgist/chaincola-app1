-- Apply instant_sell_crypto_v2 function directly
-- This migration creates the atomic instant sell function
...
BEGIN
  ...
  -- Get user asset balance based on asset type
  CASE p_asset
...
BEGIN
  IF NOT EXISTS (
    ...
    AND p.proname = 'instant_sell_crypto_v2'
    AND pg_get_function_identity_arguments(p.oid) = 'uuid, text, numeric, numeric, numeric, numeric, numeric'
  ) THEN
...
-- Add comment
COMMENT ON FUNCTION public.instant_sell_crypto_v2 IS 'Atomically swaps crypto to NGN instantly. Updates user_wallets and system_wallets, inserts transaction record. All in one transaction.';
