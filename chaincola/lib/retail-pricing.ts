/**
 * Busha-style retail display / quotes:
 * - Holdings & buy UI → higher rate (`price_ngn` / `ask`)
 * - Sell UI → lower rate (`bid`)
 * Stablecoins use a much smaller spread so pegged assets stay sane.
 */

/** ~5.2% holdings value vs instant sell payout (real-world anecdote on retail apps). */
...
export interface EngineBuySellPartial {
  buy?: number | null;
  sell?: number | null;
  /** Admin retail margin: buy_quote = sell_quote * (1 + fraction). */
...
export interface PricingRowLike {
  ...
  source?: string;
  /** NGN per 1 USD from the price feed (`get-token-prices` `usd_to_ngn`). */
  ngn_per_usd?: number;
}
...
/** Merge admin engine + venue bid/ask so wallet/buy quote high, sell quote low. */
